...
# Install equivalent packages to the Alpine base
RUN apt-get update && apt-get install -y \
      ...
      cmake \
      unzip \
      # i386 multiarch libs for Xerox driver
      libcups2:i386 \
      libcupsimage2:i386 \
      libc6:i386 \
    && rm -rf /var/lib/apt/lists/*

# Enable i386 multiarch (must be done before apt-get update above,
# so we split into two RUN commands)
RUN dpkg --add-architecture i386

# Re-run apt-get update and install after adding i386 arch
RUN apt-get update && apt-get install -y \
      libcups2:i386 \
      libcupsimage2:i386 \
      libc6:i386
